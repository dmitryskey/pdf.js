  ...
  static _create(xref, ref, pdfManager, idFactory, acroForm, evaluator, task) {
    ...
    switch (subtype) {
      ...
      case "Widget":
        ...
        return widget.setWidgetAnnotationDefaultAppearance(parameters);

      case "Popup":
        return Promise.resolve(new PopupAnnotation(parameters));

      case "FreeText":
        return Promise.resolve(new FreeTextAnnotation(parameters));
      ...
      case "Caret":
        return Promise.resolve(new CaretAnnotation(parameters));

      case "Ink":
        return Promise.resolve(new InkAnnotation(parameters));
...
class WidgetAnnotation extends Annotation {
  ...
  /**
   * Set the widget annotion default appearance (such as fonts and colors).
   *
   * @public
   * @memberof WidgetAnnotation
   * @param {number} parameters - Parsing parameters
   * @return {Promise}
   */
  setWidgetAnnotationDefaultAppearance(parameters) {
    if (!parameters.acroForm || !parameters.evaluator) {
      return Promise.resolve(this);
    }

    const data = this.data;

    const opList = new OperatorList(null, null, null, parameters.acroForm);
    const appearanceStream = new Stream(stringToBytes(data.defaultAppearance));
    const formFonts = parameters.acroForm.get("DR");
    return parameters.evaluator
      .getOperatorList({
          ...
          switch (fn | 0) {
            ...
            case OPS.setGrayFill:
              if (arg.length < 1) {
                ...
                const gray = Math.round(arg[0] * 0x100);
            ...
            case OPS.setFillRGBColor:
              if (arg.length < 3) {
                ...
                data.fontColor = Util.makeCssRgb(arg[0], arg[1], arg[2]);
              }
              break;
          }
        }
